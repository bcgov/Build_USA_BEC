## See Chapter 6 in ML algorithms for soils mapping page 228 for description of scripts------------------------------------------------------------------------
library(plotKML)
library(sp)
...
## ------------------------------------------------------------------------
library(plotKML)
USA <- fread("./inputs/training_pts/TestUSA_Training.csv")
coordinates(USA) <- ~LON+LAT
proj4string(USA) <- CRS("+init=epsg:4326")
plot(USA)


zip.file <- "./inputs/ClimateNA_Reference.zip"
data.dir <-"./outputs"

unzip(zipfile = zip.file, exdir = data.dir)

file = list.files(data.dir, ".asc", recursive = T, full.names = T)

tif.lst = list.files("./inputs/", pattern=".asc", full.names=TRUE)
tif = raster::Stack(tif.lst)
tif = as(tif, "SpatialGridDataFrame")
tif.mean = tif[1]
tif.mean@data[,1] = rowMeans(tif@data, na.rm = TRUE)
library(rgdal)
writeGDAL(tif.mean[1], "D:/PEM/mean_error.tif")

## ------------------------------------------------------------------------
eberg_spc <- spc(eberg_grid, ~ PRMGEO6+DEMSRT6+TWISRT6+TIRAST6) # factors to indicators and then PCa)
eberg_grid@data <- cbind(eberg_grid@data, eberg_spc@predicted@data)

spplot(eberg_spc@predicted)
## ------------------------------------------------------------------------
...
## This drops the classes with fewer than 5 samples------------------------------------------------------------------------
...
## should replace with ranger which is faster - randomForest may nlo longer be maintained------------------------------------------------------------------------
## subset to speed-up:
...
TAXGRSC.mn <- nnet::multinom(fm, m) # neural net
TAXGRSC.svm <- e1071::svm(fm, m, probability=TRUE, cross=5) # support vector machine
...
g1 <-  eberg_grid[1] # create empty grid
g1@data =  data.frame(probs1) # add prediction probabilities from rF
spplot(g1)

g2 <-  eberg_grid[1] # create empty grid
g2@data =  data.frame(probs2) # add prediction probabilities from nn
spplot(g2)

g3 <- eberg_grid[1] # create empty grid
g3@data =  data.frame(probs3) # add prediction probabilities from svm
spplot(g3)

plotKML(eberg ["soiltype"])

## the is suboptimal since it does not take the best MLA but the average between all------------------------
# better to use super learner - meta learner model that assigns weight based on ML accuracy
leg <- levels(m$soiltype)
lt <- list(probs1[,leg], probs2[,leg], probs3[,leg])
probs <- Reduce("+", lt) / length(lt) #REDUCE is function v efficient to sum all the list  averges the SD
...
## if have probabilities return the class with the max probability------------------------------------------------------------------------
...
## ---- message=FALSE------------------------------------------------------
library(h2o) # has fuzzy kmeans, development in java
localH2O = h2o.init(startH2O=TRUE) # need updated Java
...
listWrappers() # 41 different models
...
sl <- snowSuperLearner(Y = mP3$PHIHOX, 
                       ...
                       SL.library = sl.l)
sl
#The lower the Risk the better the model and the Coeff is the weighting SL will use in blending the models
## ------------------------------------------------------------------------
...
## THis example includes location (spatial cross validation)----------------------------------------
str(rm.cookfarm$SOURCEID)
cv_sl <- CV.SuperLearner(Y = mP3$PHIHOX, 
                         X = mP3[,all.vars(fm.PHI)[-1]],
                         parallel = cl, 
                         SL.library = sl.l2, 
                         V=5, id=rm.cookfarm$SOURCEID[rc], ### never use the same id for training and crossvalidate
                         ###this is where we would use the sample site as the ID
                         verbose=TRUE)
summary(cv_sl)
